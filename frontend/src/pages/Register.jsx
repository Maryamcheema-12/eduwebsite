...
const Register = () => {
  ...
  return (
    <div className="min-h-screen bg-white selection:bg-amber-100 overflow-x-hidden pt-20 md:pt-24">
      
      {/* --- 3D HERO --- */}
      <section ref={heroRef} onMouseMove={handleMouseMove} onMouseLeave={() => { x.set(0); y.set(0); }} className="relative py-4 md:py-10 lg:py-32 bg-white border-b border-slate-200 perspective-[1500px]">
        ...
        <div className="container mx-auto px-4 md:px-6 lg:px-12 relative z-10">
          <div className="grid lg:grid-cols-2 gap-8 md:gap-10 lg:gap-14 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
              <div className="inline-flex items-center gap-3 md:gap-4 mb-4 md:mb-6 bg-slate-950 text-white px-6 md:px-8 py-3 md:py-4 rounded-lg">
                <Activity className="w-5 md:w-6 h-5 md:h-6 text-amber-500 animate-pulse" />
                <span className="text-xs md:text-sm font-black uppercase tracking-widest">Institutional Intake 2026</span>
              </div>
              <h1 className="text-5xl md:text-6xl lg:text-7xl font-black text-slate-900 mb-4 md:mb-6 lg:mb-8 uppercase leading-tight tracking-tight">Secure <br /><span className="text-amber-600 underline decoration-slate-200 underline-offset-4 md:underline-offset-6 lg:underline-offset-8">Entry.</span></h1>
              <p className="text-lg md:text-xl text-slate-600 font-medium leading-relaxed md:leading-8 max-w-2xl">Official enrollment for specialized Engineering licensure programs in Qatar. Your professional validation begins here.</p>
            </motion.div>
      ...
      {/* --- INTERFACE --- */}
      <section className="py-4 md:py-10 lg:py-32 border-t border-slate-200">
        <div className="container mx-auto px-4 sm:px-5 md:px-6 lg:px-12">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 md:gap-10 lg:gap-12">
            ...
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="lg:col-span-5">
              <div className="rounded-2xl sm:rounded-3xl md:rounded-4xl overflow-hidden shadow-2xl border border-slate-100 h-[400px] sm:h-[450px] md:h-[500px] lg:h-[550px] relative group sticky top-32">
                <iframe 
                  title="Doha Hub"
                ...
                <div className="absolute bottom-6 sm:bottom-8 md:bottom-10 left-6 sm:left-8 md:left-10 right-6 sm:right-8 md:right-10">
                    <div className="bg-slate-950/95 backdrop-blur-lg p-4 sm:p-5 md:p-6 rounded-2xl sm:rounded-3xl md:rounded-4xl text-white shadow-2xl">
                        <MapPin className="text-amber-500 mb-2 sm:mb-3 md:mb-4" size={32} />
                        <h4 className="font-black text-lg sm:text-xl md:text-2xl text-white uppercase tracking-tight mb-1 sm:mb-1.5 md:mb-2">Doha Head Office</h4>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="lg:col-span-7">
              <div className="bg-white border border-slate-100 p-5 sm:p-6 md:p-8 lg:p-10 rounded-2xl sm:rounded-3xl md:rounded-4xl shadow-xl">
                <AnimatePresence mode="wait">
                  {submitted ? (
                    <motion.div key="success" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center py-12 sm:py-16 md:py-20">
                      <div className="w-24 sm:w-28 md:w-32 h-24 sm:h-28 md:h-32 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-5 md:mb-6"><CheckCircle2 className="w-12 sm:w-14 md:w-16 h-12 sm:h-14 md:h-16 text-green-600" /></div>
                      <h3 className="text-5xl sm:text-6xl md:text-7xl font-black text-slate-900 mb-3 sm:mb-4 md:mb-5 tracking-tight">Data Sent.</h3>
                      <p className="text-base sm:text-lg md:text-xl text-slate-600 max-w-lg mx-auto font-medium leading-relaxed mb-6 sm:mb-8 md:mb-10">Continue the conversation on WhatsApp to finalize your seat.</p>
                      <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setSubmitted(false)} className="mt-4 sm:mt-5 md:mt-6 bg-slate-900 text-white px-8 sm:px-10 md:px-12 py-4 sm:py-5 md:py-6 rounded-xl sm:rounded-2xl md:rounded-3xl font-black uppercase text-xs sm:text-sm md:text-base tracking-[0.15em] sm:tracking-[0.2em] shadow-lg hover:bg-amber-600 transition-all">New Entry</motion.button>
                    </motion.div>
                  ) : (
                    <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8 md:space-y-10 lg:space-y-12">
                      <div className="grid md:grid-cols-2 gap-5 sm:gap-6 md:gap-8 lg:gap-10">
                      ...
                      <div className="space-y-3 sm:space-y-4 md:space-y-5">
                        ...
                        <div className="relative group cursor-pointer">
                          ...
                          <div className="border-2 border-dashed border-slate-300 rounded-2xl sm:rounded-3xl md:rounded-4xl p-8 sm:p-10 md:p-12 text-center group-hover:border-amber-500 group-hover:bg-amber-50/30 transition-all bg-slate-50">
                            <Upload className="w-12 sm:w-14 md:w-16 h-12 sm:h-14 md:h-16 text-slate-400 mx-auto mb-3 sm:mb-4 md:mb-5 group-hover:text-amber-600 transition-colors" />
                        ...
                        <div className="grid grid-cols-2 gap-3 sm:gap-4 md:gap-5">
                          {files.map((file, i) => (
                            <div key={i} className="flex items-center justify-between p-2 sm:p-3 md:p-4 bg-slate-900 rounded-lg sm:rounded-xl md:rounded-2xl text-white">
                      ...
                      <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="submit" disabled={loading} className="w-full bg-slate-950 text-white py-4 sm:py-5 md:py-6 lg:py-7 rounded-xl sm:rounded-2xl md:rounded-3xl font-black uppercase tracking-[0.2em] sm:tracking-[0.25em] md:tracking-[0.3em] text-xs sm:text-sm md:text-base transition-all hover:bg-amber-600 disabled:opacity-50 flex items-center justify-center gap-2 sm:gap-3 shadow-lg">
      ...
      {/* --- TRUST --- */}
      <section className="py-4 md:py-10 lg:py-32 border-t border-slate-200 container mx-auto px-4 md:px-6 text-center opacity-40 grayscale hover:opacity-100 hover:grayscale-0 transition-all duration-1000">
